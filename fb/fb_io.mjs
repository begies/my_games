...
function fb_deleteRec(_path, _key, _procFunc) {
    ...
    remove(FB_DBREF).then(() => {
        _procFunc('OK', _path, _key, null);
    }).catch((error) => {
        _procFunc('error', _path, _key, error);
    });
}
...
function fb_readSortedLimit(_path, _procFunc, _save, _sortKey, _num) {
    ...
    get(FB_DBREF).then((snapshot) => {
        ...
        _procFunc(fb_temp, _path, snapshot, _save, null, _sortKey, _num);
    }).catch((error) => {
        _procFunc('error', _path, null, _save, error, _sortKey, _num);
    });
}
